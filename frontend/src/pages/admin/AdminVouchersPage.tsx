...
const voucherStatusMeta: Record<string, { label: string; badgeClass: string }> = {
  ACTIVE: { label: "Active", badgeClass: "border-emerald-500/30 bg-emerald-500/10 text-emerald-700" },
  INACTIVE: { label: "Inactive", badgeClass: "border-slate-500/30 bg-slate-500/10 text-slate-700" },
  EXPIRED: { label: "Expired", badgeClass: "border-rose-500/30 bg-rose-500/10 text-rose-700" },
};
...
function getVoucherStatusMeta(status: string) {
  return voucherStatusMeta[status] ?? { label: "Unknown", badgeClass: "border bg-background text-muted-foreground" };
}
  ...
  async function save() {
    ...
    if (!editingId) {
      if (!form.code.trim() || !form.name.trim() || !form.discountType || !form.startAt.trim() || !form.endAt.trim()) {
        toast.push({ variant: "error", title: "Missing required fields", message: "Code, name, discount type, start date, and end date are required." });
        return;
      }
      ...
      <Card className="border bg-background shadow-sm">
        ...
        <CardContent className="space-y-4">
          <div className="grid gap-3 md:grid-cols-4">
            <Input value={qCode} onChange={(e) => setQCode(e.target.value)} placeholder="Code" className="rounded-md" />
            <Input value={qName} onChange={(e) => setQName(e.target.value)} placeholder="Name" className="rounded-md" />
            <select title="Select option" value={qStatus} onChange={(e) => setQStatus(e.target.value)} className="h-10 rounded-md border bg-background px-3 text-sm">
              <option value="">All statuses</option>
              {voucherStatuses.map((s) => (
                <option key={s} value={s}>
                  {getVoucherStatusMeta(s).label}
                </option>
              ))}
            </select>
            <select title="Select option" value={qDiscountType} onChange={(e) => setQDiscountType(e.target.value)} className="h-10 rounded-md border bg-background px-3 text-sm">
              ...
              <tbody>
                {isLoading ? (
                  ...
                  items.map((v) => {
                    ...
                    const statusMeta = getVoucherStatusMeta(status);
                    return (
                      <tr key={String(id)} className="border-t">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex justify-end gap-2">
                            ...
                            <Button variant="outline" className="h-9 rounded-md" onClick={() => openEdit(v)} disabled={!id}>
                              Edit
                            </Button>
                            <Button
                              variant="outline"
      ...
      <Modal isOpen={isFormOpen} title={editingId ? `Edit voucher ${form.name || form.code || ""}`.trim() : "New voucher"} onClose={() => setIsFormOpen(false)}>
        <div className="grid gap-3">
          ...
          <div className="grid gap-3 md:grid-cols-2">
            <select title="Select option" value={form.discountType} onChange={(e) => setForm((f) => ({ ...f, discountType: e.target.value }))} className="h-10 rounded-md border bg-background px-3 text-sm">
              {discountTypes.map((t) => (
                <option key={t} value={t}>
                  {discountTypeLabel[t]}
                </option>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            <Input value={form.discountValue} onChange={(e) => setForm((f) => ({ ...f, discountValue: e.target.value }))} placeholder="Discount value" className="rounded-md" />
            <Input value={form.maxDiscountAmount} onChange={(e) => setForm((f) => ({ ...f, maxDiscountAmount: e.target.value }))} placeholder="Max discount" className="rounded-md" />
            <Input value={form.minOrderTotal} onChange={(e) => setForm((f) => ({ ...f, minOrderTotal: e.target.value }))} placeholder="Min order total" className="rounded-md" />
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <Input value={form.startAt} onChange={(e) => setForm((f) => ({ ...f, startAt: e.target.value }))} placeholder="Start at * (YYYY-MM-DDTHH:mm:ss)" className="rounded-md" />
            <Input value={form.endAt} onChange={(e) => setForm((f) => ({ ...f, endAt: e.target.value }))} placeholder="End at * (YYYY-MM-DDTHH:mm:ss)" className="rounded-md" />
          </div>
          <div className="grid gap-3 md:grid-cols-2">
            <Input value={form.usageLimitGlobal} onChange={(e) => setForm((f) => ({ ...f, usageLimitGlobal: e.target.value }))} placeholder="Usage limit global" className="rounded-md" />
            <Input value={form.usageLimitUser} onChange={(e) => setForm((f) => ({ ...f, usageLimitUser: e.target.value }))} placeholder="Usage limit per user" className="rounded-md" />
          </div>
