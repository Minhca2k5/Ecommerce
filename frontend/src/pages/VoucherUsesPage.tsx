    ...
    <div className="space-y-6">
      ...
      <Card className="bg-background">
        ...
        <CardContent className="grid gap-3 sm:grid-cols-3">
          <div className="space-y-2">
            ...
            <Input className="rounded-xl bg-background" value={orderIdFilter} onChange={(e) => setOrderIdFilter(e.target.value)} inputMode="numeric" placeholder="e.g. 30" />
          </div>
          <div className="space-y-2">
            <div className="text-xs font-medium text-muted-foreground">Voucher ID</div>
            <Input className="rounded-xl bg-background" value={voucherIdFilter} onChange={(e) => setVoucherIdFilter(e.target.value)} inputMode="numeric" placeholder="e.g. 1" />
          </div>
      ...
      {items.length === 0 ? (
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          {items.map((u) => (
            <Card key={String(u.id)} className="pressable overflow-hidden bg-background shadow-sm transition hover:shadow-md">
